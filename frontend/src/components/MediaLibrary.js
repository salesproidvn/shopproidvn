...
export default function MediaLibrary({ open, onClose, onSelect, multiple = false, maxSelect = 1 }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col p-0" hideClose>
        <DialogHeader className="px-5 pt-5 pb-3 border-b flex-shrink-0">
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-lg font-bold">Thư viện ảnh</DialogTitle>
              <DialogDescription className="text-xs text-[#94A3B8]">Chọn ảnh từ thư viện hoặc upload mới</DialogDescription>
            </div>
            <div className="flex items-center gap-2">
              <input type="file" ref={fileInputRef} onChange={handleUpload} accept="image/*" multiple className="hidden" />
              <Button type="button" size="sm" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="text-xs gap-1" data-testid="media-upload-btn">
        ...
        <div className="flex-1 overflow-y-auto p-4" data-testid="media-grid-container">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-[#94A3B8]">
              ...
              <p className="text-xs">Upload ảnh mới để bắt đầu</p>
            </div>
          ) : (
            ...
            <div className="flex justify-center gap-2 mt-4">
              {Array.from({ length: totalPages }, (_, i) => (
                <Button key={i} type="button" variant={page === i + 1 ? 'default' : 'outline'} size="sm"
                  className="w-8 h-8 text-xs" onClick={() => fetchMedia(i + 1)}>
                  {i + 1}
                </Button>
              ))}
            </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t bg-[#F8FAFC] flex-shrink-0">
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" size="sm" onClick={handleConfirm} disabled={!selected.length} className="text-xs gap-1" data-testid="media-confirm-btn">
